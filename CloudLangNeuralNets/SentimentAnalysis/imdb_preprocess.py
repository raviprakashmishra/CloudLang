...
tokenizer_cmd = [sys.executable,'./tokenizer.perl', '-l', 'en', '-q', '-']
...
def tokenize(sentences):
    ...
    #tok_text = bytes(tok_text, 'UTF-8')
    toks = tok_text.decode('utf8').split('\n')[:-1]
...
def build_dict(path):
    sentences = []
    currdir = os.getcwd()
    print ('currentwork dir is '+currdir)
    print ('currentwork dir is '+currdir+path)
    total = currdir+path
    os.chdir('%s/pos/' %total)
    for ff in glob.glob("*.txt"):
        with open(ff, 'r',encoding="utf8") as f:
            sentences.append(f.readline().strip())
    os.chdir('%s/neg/' % total)
    for ff in glob.glob("*.txt"):
        with open(ff, 'r',encoding="utf8") as f:
            sentences.append(f.readline().strip())
    os.chdir(currdir)
